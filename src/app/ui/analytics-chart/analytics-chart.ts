...
export class AnalyticsChartComponent {
  ...
  approvedHeight = computed(() => {
   const count = this.data().filter((e: Enrollment) => e.status === 'Approved').length;
    return Math.max(20, count * 3);
  });

  pendingHeight = computed(() => {
    const count = this.data().filter((e: Enrollment) => e.status === 'Pending').length;
    return Math.max(20, count * 3);
  });

  rejectedHeight = computed(() => {
    const count = this.data().filter((e: Enrollment) => e.status === 'Rejected').length;
    return Math.max(20, count * 3);
  });
}